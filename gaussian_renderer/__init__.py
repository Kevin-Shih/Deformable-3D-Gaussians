...
def render_laplacian(viewpoint_camera, pc: LaplacianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling, is_6dof=False,
           scaling_modifier=1.0, override_color=None):
    """
    Render the scene.
    ...
    if pipe.compute_cov3D_python:
        ...
        scales = pc.get_scaling*pc.get_shape + d_scaling
        rotations = pc.get_rotation + d_rotation
...
def render_laplacian_static(viewpoint_camera, pc : LaplacianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene. 
